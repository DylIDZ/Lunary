...
local Release = "Prerelease Beta 6.1 - Enhanced"
...
local Luna = { 
	...
	ThemeGradient = ColorSequence.new{
		...
		ColorSequenceKeypoint.new(0.35, Color3.fromRGB(75, 0, 130)),
...
local Platform = {
	...
	ScreenSize = Vector2.new(1,1),
	Orientation = "Portrait"
}

local function UpdatePlatformInfo()
	local cam = Camera
	...
	Platform.IsPC = UserInputService.KeyboardEnabled
	Platform.Orientation = screenSize.X > screenSize.Y and "Landscape" or "Portrait"
end
...
function TouchGestureSystem:RegisterGesture(GestureName, Callback)
	if not self.Gestures[GestureName] then
		self.Gestures[GestureName] = {}
	end
	table.insert(self.Gestures[GestureName], Callback)
end

function TouchGestureSystem:FireGesture(GestureName, Data)
	if self.Gestures[GestureName] then
		for _, Callback in ipairs(self.Gestures[GestureName]) do
			pcall(function() Callback(Data) end)
		end
	end
end

local function GetTouchDistance(touch1, touch2)
	local delta = touch1 - touch2
...
function HapticSystem:Trigger(Duration, Pattern)
	if not self.Enabled or not Platform.IsMobile then return end
	if game:FindFirstChild("HapticService") then
		pcall(function()
			game.HapticService:SetMotor(Enum.UserInputType.Touch, Enum.VibrationMotor.Large, self.Intensity)
			task.wait(Duration or 0.1)
			game.HapticService:SetMotor(Enum.UserInputType.Touch, Enum.VibrationMotor.Large, 0)
		end)
	end
end
...
function KeybindManager:Register(KeyCode, Callback, Options)
	Options = Options or {}
	
	if #self.Keybinds >= self.MaxBinds then
		return false, "Max keybinds reached"
	end
	
	local BindData = {
		KeyCode = KeyCode,
		Callback = Callback,
		HoldToActivate = Options.HoldToActivate or false,
		Activated = false,
		CreatedAt = tick()
	}
	
	table.insert(self.Keybinds, BindData)
	return true, BindData
end

function KeybindManager:Unregister(KeyCode)
...
function KeybindManager:GetAll()
	return self.Keybinds
end

function KeybindManager:Execute(KeyCode)
	for _, Bind in ipairs(self.Keybinds) do
		if Bind.KeyCode == KeyCode then
			pcall(function() Bind.Callback() end)
			return true
		end
	end
	return false
...
function PerformanceOptimizer:OptimizeTween(Instance, TweenInfo, Properties)
	...
	local tween = CreateOptimizedTween(Instance, TweenInfo, Properties)
	table.insert(self.TweenConnections, tween)
	tween:Play()
	
	tween.Completed:Connect(function()
		for i, t in ipairs(self.TweenConnections) do
			if t == tween then
				table.remove(self.TweenConnections, i)
				break
			end
		end
	end)
...
function PerformanceOptimizer:ClearCache()
	table.clear(self.CachedCalculations)
	table.clear(TweenCache)
end

Luna.Platform = Platform
Luna.TouchGestures = TouchGestureSystem
Luna.Haptics = HapticSystem
Luna.NotificationQueue = NotificationQueue
Luna.Keybinds = KeybindManager
Luna.Performance = PerformanceOptimizer

function Luna:ApplyTheme(UIElement)
	if not UIElement then return end
	
	if UIElement:FindFirstChild("UIStroke") then
		local stroke = UIElement.UIStroke
		if stroke.Transparency > 0.7 then
			stroke.Color = Luna.Colors.Border
			stroke.Transparency = 0.5
		elseif stroke.Transparency <= 0.1 then
			stroke.Color = Luna.Colors.Primary
		end
	end
	
	if UIElement.ClassName == "Frame" then
		if UIElement.BackgroundTransparency < 0.5 then
			UIElement.BackgroundColor3 = Luna.Colors.Surface
		elseif UIElement.BackgroundTransparency >= 0.5 then
			UIElement.BackgroundColor3 = Luna.Colors.DarkBg
		end
	end
	
	if UIElement:IsA("TextLabel") or UIElement:IsA("TextButton") then
		UIElement.TextColor3 = Luna.Colors.Text
	end
end

function Luna:ApplyThemeRecursive(UIElement)
	Luna:ApplyTheme(UIElement)
	for _, child in ipairs(UIElement:GetChildren()) do
		Luna:ApplyThemeRecursive(child)
	end
end

function Luna:AddShadowDepth(UIElement, depth)
	depth = depth or 0.15
	if UIElement:FindFirstChild("UIStroke") then
		local stroke = UIElement.UIStroke
...
function Luna:EnhanceElementAnimation(element, animationType)
	animationType = animationType or "smooth"
	if animationType == "smooth" then
		return TweenInfo.new(0.3, Enum.EasingStyle.Quad, Enum.EasingDirection.Out)
	elseif animationType == "snappy" then
		return TweenInfo.new(0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.Out)
	elseif animationType == "slow" then
		return TweenInfo.new(0.5, Enum.EasingStyle.Quad, Enum.EasingDirection.InOut)
	end
end

function Luna:EnhanceTextLabel(element)
	if element:IsA("TextLabel") or element:IsA("TextButton") then
		element.TextColor3 = Luna.Colors.Text
		element.TextScaled = false
		element.TextSize = element.TextSize or 14
		if element.TextStrokeTransparency == 1 then
			element.TextStrokeColor3 = Luna.Colors.DarkBg
			element.TextStrokeTransparency = 0.8
		end
	end
end

function Luna:EnhanceBorder(element, thickness, color)
	thickness = thickness or 1
	color = color or Luna.Colors.Primary
...
RunService:BindToRenderStep("LunaUpdatePlatform", Enum.RenderPriority.Input.Value + 1, UpdatePlatformInfo)
...
function tween(object, goal, callback, tweenin)
	local customTweenInfo = tweenin or TweenInfo.new(0.35, Enum.EasingStyle.Quad, Enum.EasingDirection.Out)
	local tween = TweenService:Create(object, customTweenInfo, goal)
...
function Luna:Notification(data) -- action e.g open messages
	task.spawn(function()
		...
		newNotification.BackgroundColor3 = Luna.Colors.Surface
		newNotification.UIStroke.Color = Luna.Colors.Primary
		newNotification.Title.TextColor3 = Luna.Colors.Text
		newNotification.Description.TextColor3 = Luna.Colors.TextMuted
		...
		newNotification.Size = UDim2.new(1, 0, 0, -Notifications:FindFirstChild("UIListLayout").Padding.Offset)
		TweenService:Create(newNotification, TweenInfo.new(0.4, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {Size = UDim2.new(1, 0, 0, bounds)}):Play()

		task.wait(0.12)
		TweenService:Create(newNotification, TweenInfo.new(0.35, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {BackgroundTransparency = 0.15}):Play()
		TweenService:Create(newNotification.Title, TweenInfo.new(0.25, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {TextTransparency = 0}):Play()

		task.wait(0.05)

		TweenService:Create(newNotification.Icon, TweenInfo.new(0.25, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {ImageTransparency = 0}):Play()

		task.wait(0.05)
		TweenService:Create(newNotification.Description, TweenInfo.new(0.25, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {TextTransparency = 0}):Play()
		TweenService:Create(newNotification.UIStroke, TweenInfo.new(0.3, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {Transparency = 0.3}):Play()
		TweenService:Create(newNotification.Shadow, TweenInfo.new(0.25, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {ImageTransparency = 0.7}):Play()
	...
	function Window:CreateTab(TabSettings)
		...
		function Tab:CreateSection(name : string)
			...
			function Section:CreateSlider(SliderSettings, Flag)
				...
				PerformanceOptimizer:OptimizeTween(Slider, TweenInfo.new(0.7, Enum.EasingStyle.Exponential), {BackgroundTransparency = 0.5})
				PerformanceOptimizer:OptimizeTween(Slider.UIStroke, TweenInfo.new(0.7, Enum.EasingStyle.Exponential), {Transparency = 0.5})
				PerformanceOptimizer:OptimizeTween(Slider.Title, TweenInfo.new(0.7, Enum.EasingStyle.Exponential), {TextTransparency = 0})	
				...
				if Platform.IsMobile then
					Slider.UIStroke.Color = Color3.fromRGB(87, 84, 104)
				end

				Slider.Interact.InputBegan:Connect(function(Input)
					if Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.Touch then 
						SLDragging = true 
						HapticSystem:Trigger(0.05)
					end 
				end)
			...
			function Section:CreateToggle(ToggleSettings, Flag)    
				...
				local function ToggleClick()
					...
					HapticSystem:Trigger(0.08)

					local Success, Response = pcall(function()
						ToggleSettings.Callback(ToggleSettings.CurrentValue)
					end)
					if not Success then
						PerformanceOptimizer:OptimizeTween(Toggle, TweenInfo.new(0.4, Enum.EasingStyle.Quad), {BackgroundTransparency = 0})
						PerformanceOptimizer:OptimizeTween(Toggle, TweenInfo.new(0.4, Enum.EasingStyle.Quad), {BackgroundColor3 = Luna.Colors.Error})
						PerformanceOptimizer:OptimizeTween(Toggle.UIStroke, TweenInfo.new(0.4, Enum.EasingStyle.Quad), {Transparency = 0})
						Toggle.Title.Text = "Error"
						print("Luna Interface Suite | "..ToggleSettings.Name.." Callback Error " ..tostring(Response))
						wait(0.5)
						Toggle.Title.Text = ToggleSettings.Name
						PerformanceOptimizer:OptimizeTween(Toggle, TweenInfo.new(0.4, Enum.EasingStyle.Quad), {BackgroundTransparency = 0.2})
						PerformanceOptimizer:OptimizeTween(Toggle, TweenInfo.new(0.4, Enum.EasingStyle.Quad), {BackgroundColor3 = Luna.Colors.DarkBg})
						PerformanceOptimizer:OptimizeTween(Toggle.UIStroke, TweenInfo.new(0.4, Enum.EasingStyle.Quad), {Transparency = 0.5})
					end
				end

				Toggle.Interact.MouseButton1Click:Connect(ToggleClick)
				...
				if not Platform.IsMobile then
					Toggle["MouseEnter"]:Connect(function()
						tween(Toggle.UIStroke, {Color = Luna.Colors.Primary, Transparency = 0})
						tween(Toggle, {BackgroundColor3 = Luna.Colors.Surface})
					end)

					Toggle["MouseLeave"]:Connect(function()
						tween(Toggle.UIStroke, {Color = Luna.Colors.Border, Transparency = 0.5})
						tween(Toggle, {BackgroundColor3 = Luna.Colors.DarkBg})
					end)
				else
					Toggle.UIStroke.Color = Luna.Colors.Primary
		...
		function Tab:CreateButton(ButtonSettings)
			...
			Button.UIStroke.Transparency = 1
			Button.Title.TextTransparency = 1
			Button.BackgroundColor3 = Luna.Colors.DarkBg
			Button.UIStroke.Color = Luna.Colors.Border
			if ButtonSettings.Description ~= nil and ButtonSettings.Description ~= "" then
				Button.Desc.TextTransparency = 1
			end

			PerformanceOptimizer:OptimizeTween(Button, TweenInfo.new(0.5, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {BackgroundTransparency = 0.2})
			PerformanceOptimizer:OptimizeTween(Button.UIStroke, TweenInfo.new(0.5, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {Transparency = 0.5})
			PerformanceOptimizer:OptimizeTween(Button.Title, TweenInfo.new(0.5, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {TextTransparency = 0})	
			if ButtonSettings.Description ~= nil and ButtonSettings.Description ~= "" then
				PerformanceOptimizer:OptimizeTween(Button.Desc, TweenInfo.new(0.5, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {TextTransparency = 0})	
			end
			...
			local function ButtonCallback()
				...
				if not Success then
					PerformanceOptimizer:OptimizeTween(Button, TweenInfo.new(0.4, Enum.EasingStyle.Quad), {BackgroundTransparency = 0})
					PerformanceOptimizer:OptimizeTween(Button, TweenInfo.new(0.4, Enum.EasingStyle.Quad), {BackgroundColor3 = Luna.Colors.Error})
					PerformanceOptimizer:OptimizeTween(Button.UIStroke, TweenInfo.new(0.4, Enum.EasingStyle.Quad), {Transparency = 0})
					Button.Title.Text = "Error"
					print("Luna Interface Suite | "..ButtonSettings.Name.." Callback Error " ..tostring(Response))
					wait(0.5)
					Button.Title.Text = ButtonSettings.Name
					PerformanceOptimizer:OptimizeTween(Button, TweenInfo.new(0.4, Enum.EasingStyle.Quad), {BackgroundTransparency = 0.2})
					PerformanceOptimizer:OptimizeTween(Button, TweenInfo.new(0.4, Enum.EasingStyle.Quad), {BackgroundColor3 = Luna.Colors.DarkBg})
					PerformanceOptimizer:OptimizeTween(Button.UIStroke, TweenInfo.new(0.4, Enum.EasingStyle.Quad), {Transparency = 0.5})
				else
					tween(Button.UIStroke, {Color = Luna.Colors.Success})
					wait(0.15)
					if ButtonV.Hover then
						tween(Button.UIStroke, {Color = Luna.Colors.Primary})
					else
						tween(Button.UIStroke, {Color = Luna.Colors.Border})
					end
				end
			end

			Button.Interact["MouseButton1Click"]:Connect(ButtonCallback)
			...
			if not Platform.IsMobile then
				...
				Button["MouseLeave"]:Connect(function()
					ButtonV.Hover = false
					tween(Button.UIStroke, {Color = Luna.Colors.Border, Transparency = 0.5})
					tween(Button, {BackgroundColor3 = Luna.Colors.DarkBg})
				end)
			else
				Button.UIStroke.Color = Luna.Colors.Primary
		...
		function Tab:CreateToggle(ToggleSettings, Flag)    
			...
			Toggle.UIStroke.Transparency = 1
			Toggle.Title.TextTransparency = 1
			Toggle.BackgroundColor3 = Luna.Colors.DarkBg
			Toggle.UIStroke.Color = Luna.Colors.Border
			if ToggleSettings.Description ~= nil and ToggleSettings.Description ~= "" then
				Toggle.Desc.TextTransparency = 1
			end

			TweenService:Create(Toggle, TweenInfo.new(0.5, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {BackgroundTransparency = 0.2}):Play()
			if ToggleSettings.Description ~= nil and ToggleSettings.Description ~= "" then
				TweenService:Create(Toggle.Desc, TweenInfo.new(0.5, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {TextTransparency = 0}):Play()
			end
			TweenService:Create(Toggle.UIStroke, TweenInfo.new(0.5, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {Transparency = 0.5}):Play()
			TweenService:Create(Toggle.Title, TweenInfo.new(0.5, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {TextTransparency = 0}):Play()	

			local function Set(bool)
				if bool then
					Toggle.toggle.color.Enabled = true
					tween(Toggle.toggle, {BackgroundTransparency = 0, BackgroundColor3 = Luna.Colors.Success})

					Toggle.toggle.UIStroke.color.Enabled = true
					tween(Toggle.toggle.UIStroke, {Color = Luna.Colors.Primary, Transparency = 0})

					tween(Toggle.toggle.val, {BackgroundColor3 = Luna.Colors.Text, Position = UDim2.new(1,-23,0.5,0), BackgroundTransparency = 0.3})
				else
					Toggle.toggle.color.Enabled = false
					Toggle.toggle.UIStroke.color.Enabled = false

					Toggle.toggle.UIStroke.Color = Luna.Colors.Border
					Toggle.toggle.BackgroundColor3 = Luna.Colors.Surface

					tween(Toggle.toggle, {BackgroundTransparency = 0.8})

					tween(Toggle.toggle.val, {BackgroundColor3 = Luna.Colors.TextMuted, Position = UDim2.new(0,5,0.5,0), BackgroundTransparency = 0.5})
				end
...
Luna.MobileUI = {
	...
	GetResponsiveSize = function(self, BaseSize)
		if Platform.IsMobile then
			if Platform.Orientation == "Portrait" then
				return math.min(BaseSize, self.ResponsiveSizes.PortraitWidth)
			else
				return math.min(BaseSize, self.ResponsiveSizes.LandscapeWidth)
			end
		end
		return BaseSize
	end,
	
	EnableTouchAreaExpansion = function(self, UIElement, Multiplier)
		if Platform.IsMobile and UIElement then
			local originalSize = UIElement.Size
			UIElement.Size = UDim2.new(originalSize.X.Scale * (Multiplier or self.ResponsiveSizes.TouchAreaMultiplier), 
										originalSize.X.Offset, 
										originalSize.Y.Scale * (Multiplier or self.ResponsiveSizes.TouchAreaMultiplier), 
...
Luna.InputHelper = {
	ValidateInput = function(self, Text, Type)
		if Type == "Numeric" then
			return tonumber(Text) ~= nil
		elseif Type == "Alphanumeric" then
			return string.match(Text, "^[a-zA-Z0-9]*$") ~= nil
		elseif Type == "Email" then
			return string.match(Text, "^[%w%-%.]+@[%w%-%.]+%.[%w%-]+$") ~= nil
		end
		return true
	end,
	
	SanitizeInput = function(self, Text, MaxLength)
		return Text:sub(1, MaxLength or 100)
	end
}

Luna.UIHelpers = {
	CreateLoadingSpinner = function(self, Parent)
	...
	ShowToast = function(self, Message, Duration)
		Luna:Notification({
			Title = "Notification",
			Content = Message,
			Icon = "info"
	...
	DisableWhileLoading = function(self, Elements, Disabled)
		for _, Element in ipairs(Elements) do
			if Element then
				Element.Interact.Interactable = not Disabled
				Element.Transparency = Disabled and 0.5 or 0
			end
		end
	end
}
...
Luna.Analytics = {
	...
	GetEvents = function(self, EventName)
...
function Luna:SetHapticIntensity(Intensity)
	if Intensity >= 0 and Intensity <= 1 then
		HapticSystem.Intensity = Intensity
		return true
	end
	return false
end

function Luna:TriggerHaptic(Duration)
	HapticSystem:Trigger(Duration)
end

function Luna:RegisterKeybind(KeyCode, Callback, Options)
	return KeybindManager:Register(KeyCode, Callback, Options)
end

function Luna:RemoveKeybind(KeyCode)
	return KeybindManager:Unregister(KeyCode)
end

function Luna:GetAllKeybinds()
	return KeybindManager:GetAll()
end

function Luna:ClearPerformanceCache()
